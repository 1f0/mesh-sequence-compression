...
int Estimate_Geometry_Quantization(Polyhedron &pMesh, const double &volume, const double &area,
...
inline Point_Int Frenet_Rotation(const Point_Int &Dist, const Vector &T1, const Vector &T2, const Vector &normal) {
    ...
    if (CGAL::square(M.Get(0, 2)) + CGAL::square(M.Get(1, 2)) == 0)
        ...
        phi = Signe(-1 * M.Get(0, 2)) *
              std::acos(M.Get(1, 2) / std::sqrt(CGAL::square(M.Get(0, 2)) + CGAL::square(M.Get(1, 2))));
    ...
    if (CGAL::square(M.Get(1, 2)) + CGAL::square(M.Get(2, 2)) == 0)
        ...
        psi = Signe(-1 * M.Get(1, 2)) *
              std::acos(M.Get(2, 2) / std::sqrt(CGAL::square(M.Get(1, 2)) + CGAL::square(M.Get(2, 2))));
    ...
    if (CGAL::square(M.Get(0, 1)) + CGAL::square(M.Get(1, 1)) == 0)
        ...
        theta = Signe(-1 * M.Get(0, 1)) *
                std::acos(M.Get(1, 1) / std::sqrt(CGAL::square(M.Get(0, 1)) + CGAL::square(M.Get(1, 1))));
...
inline Point_Int
Inverse_Frenet_Rotation(const Point_Int &Frenet, const Vector &T1, const Vector &T2, const Vector &normal) {
    Matrix R(T1.x(), T2.x(), normal.x(), T1.y(), T2.y(), normal.y(), T1.z(), T2.z(), normal.z());
    Matrix M = R;
...
bool Is_Geometric_Metric_Violated(const Halfedge_handle &h, const int &type, const unsigned int &valence,
...
void
Retriangulation(Polyhedron &pMesh, const Halfedge_handle &ch, const unsigned &valence, const unsigned &Vertex_number,
...
bool Is_Border_Manifold_Property_Violated(const Halfedge_handle &g, const Halfedge_handle &First_border_edge,
                                          const int &Type, const int &Valence) {
    bool res = false;
...
Color_Unit Get_Average_Vertex_Color_Youn(const Halfedge_handle &h, const int &valence) {
    ...
    for (int i = 0; i < valence; i++) {
        ...
        Weights.push_back(
                Get_Angle_Weight_Youn(g)); // angle formed by front vertex and two neighbor of actual neighbor vertex
        g = g->opposite()->prev();
    }
...
double Color_Distance(const float *LAB1, const float *LAB2) {
    ...
    dist = sqrt(pow((double) LAB2[0] - (double) LAB1[0], 2.0) + pow((double) LAB2[1] - (double) LAB1[1], 2.0) +
                pow((double) LAB2[2] - (double) LAB1[2], 2.0));
    return dist;
}
