...
QString Compression_Valence_Component::Main_Function(Polyhedron &_pMesh,
    ...
    if (_Adaptive_quantization)
        this->Adaptive_Quantization(_pMesh, _NVertices, _Normal_flipping, _Use_metric, _Metric_thread,
                                    _Use_forget_metric, _Forget_value, _Qbit);
    else
        this->Simplification(_pMesh, _NVertices, _Normal_flipping, _Use_metric, _Metric_thread, _Use_forget_metric,
                             _Forget_value);
    ...
    if (_Compression_selected)
        this->Compression(_pMesh, _File_Name, _Qbit, Connectivity_size, Color_size,
                          Total_size);//, this->Initial_file_size);
            ...
            while (!facets.empty()) {
                ...
                CGAL_For_all(pHalfedge, end) {
                    ...
                    if (!Is_seed_edge_found) {
                        if (((!pHalfedge->is_border_edge()) && (!Is_Border_Vertex(pHalfedge)) &&
                             (!Is_Border_Vertex(pHalfedge->opposite()))) || (pHalfedge->next()->vertex_degree() != 6)) {
...
void Compression_Valence_Component::Quantization(Polyhedron &_pMesh) {
    ...
    for (Vertex_iterator pVert = _pMesh.vertices_begin(); pVert != _pMesh.vertices_end(); pVert++) {
        ...
        int Qx =
                (int) (ceil((x - (double) this->xmin[Component_ID]) / (double) this->Quantization_Step[Component_ID])) -
                1;
        if (Qx == -1)
            Qx = 0;
        int Qy =
                (int) (ceil((y - (double) this->ymin[Component_ID]) / (double) this->Quantization_Step[Component_ID])) -
                1;
        if (Qy == -1)
            Qy = 0;
        int Qz =
                (int) (ceil((z - (double) this->zmin[Component_ID]) / (double) this->Quantization_Step[Component_ID])) -
                1;
        if (Qz == -1)
            Qz = 0;
...
void Compression_Valence_Component::Color_Initialization(Polyhedron &_pMesh) {
    ...
    for (; pVertex != _pMesh.vertices_end(); pVertex++) {
        if ((pVertex->color(0) != this->OnlyColor[0]) || (pVertex->color(1) != this->OnlyColor[1]) ||
            (pVertex->color(2) != this->OnlyColor[2])) {
            this->IsColored = true;
            break;
        }
    }
        ...
        do {
            ...
            for (int Component_ID = 0; Component_ID < this->NumberComponents; Component_ID++) {
                ...
                if (this->ComponentOperations[Component_ID] == this->GlobalCountOperation) {
                    ...
                    int QG = Estimate_Geometry_Quantization(_pMesh, this->ComponentVolume[Component_ID],
                    ...
                    if (QG < (int) this->Qbit[Component_ID])    // MT
                    {
                        ...
                        this->Decimation_Conquest(_pMesh, _Normal_flipping, _Use_metric, _Metric_thread,
                                                  _Use_forget_metric, _Forget_value, Component_ID);
                        this->Regulation(_pMesh, _Normal_flipping, _Use_metric, _Metric_thread, _Use_forget_metric,
                                         _Forget_value, Component_ID);
        ...
        do {
            ...
            for (int Component_ID = 0; Component_ID < this->NumberComponents; Component_ID++) {
                ...
                if (this->ComponentOperations[Component_ID] == this->GlobalCountOperation) {
                    ...
                    this->Calculate_Edge_Color_Difference(_pMesh, Component_ID, Max_color, Mean_color,
                                                          Number_of_vertices);
                    double Mean_Max = Mean_color / Max_color;

                    // Estimated color quantization(QC) and geometry quantization (QG)
                    int QC = floor(
                            -1.181 * log(Mean_Max * this->ComponentArea[Component_ID] / (double) Number_of_vertices) +
                            0.3281 + 0.5);
                    int QG = Estimate_Geometry_Quantization(_pMesh, this->ComponentVolume[Component_ID],
                                                            this->ComponentArea[Component_ID],
                                                            this->ComponentNumberVertices[Component_ID]);
                    ...
                    if ((8 - this->NumberColorQuantization[Component_ID] > QC_Initials[Component_ID]) &&
                        (!this->IsOneColor))// && (QC_Final < 8 - this->NumberColorQuantization))
                    {
                        Operation_choice = 2;
                        Continue = true;
                        ...
                        // If the current color quantization precision > Estimated precision -> Decrease
                              ...
                              (QC >= QC_Finals[Component_ID])) && (!this->IsOneColor)) {
                        Operation_choice = 2;
                        Continue = true;
            ...
            if (Check_border_structure) {
                ...
                if (valence == 3) {
                    ...
                    if ((this->IsColored) && (!this->IsOneColor)) {
                        ...
                        Average_color = Get_Average_Vertex_Color_After_Removal(g,
                                                                               valence); // g is the most left placed edge
                        Color_Unit Color_diff = Removed_vertex_color - Average_color;
...
int Compression_Valence_Component::Regulation(Polyhedron &_pMesh,
    ...
    while (!Halfedges.empty()) {
        ...
        else if ((h->next()->vertex()->Vertex_Flag == FREE) && (valence == 3) &&
                 (Is_Border_Vertex(h->next()) == false)) // if valence is 3, remove the front vertex.
        {
            Halfedge_handle g = h;
...
void
Compression_Valence_Component::Write_Base_Mesh(Polyhedron &_pMesh, Arithmetic_Codec &enc, unsigned &Connectivity_size,
                                               unsigned &Color_size, const int &Num_color_base_mesh) {

    unsigned int Max_Qbit = 0;
    for (int i = 0; i < this->NumberComponents; i++) {
        enc.put_bits(this->ComponentOperations[i],
                     8);                                           // number of operations < 256
        enc.put_bits(this->Qbit[i] - 4,
                     4);                                                            // quantization bit < 16
    ...
    enc.put_bits(_pMesh.size_of_vertices(),
                 15);                                        // number of vertices of base mesh < 4096
    enc.put_bits(_pMesh.size_of_facets(),
    ...
    do {
        ...
        for (int Component_ID = 0; Component_ID < this->NumberComponents; Component_ID++) {
            ...
            if (this->ComponentOperations[Component_ID] == this->GlobalCountOperation) {
                ...
                this->Decimation_Conquest(_pMesh, Normal_flipping, Use_metric, Metric_thread, Use_forget_metric,
                                          Forget_value, Component_ID);
                this->Regulation(_pMesh, Normal_flipping, Use_metric, Metric_thread, Use_forget_metric, Forget_value,
                                 Component_ID);
...
//const unsigned & Initial_file_size)
{
    ...
    FILE *fp = fopen(File_Name,
                     "wb");                                                    //Main FILE to save compression information.

    int res;

    res = fwrite(&this->Smallest_Alpha, sizeof(int), 1,
                 fp);                             // smallest value of alpha (to save the negative value)
    res = fwrite(&this->Smallest_Gamma, sizeof(int), 1,
                 fp);                             // smallest value of gamma (to save the negative value)
    res = fwrite(&this->Initial_file_size, sizeof(unsigned), 1,
                 fp);            // Intial size of the input file (To visualize during decompression)
    res = fwrite(&this->NumberComponents, sizeof(int), 1, fp);

    for (int i = 0; i < this->NumberComponents; i++) {
        res = fwrite(&this->Quantization_Step[i], sizeof(float), 1,
                     fp);                                // Quantization_Step(step of quantization)
        res = fwrite(&this->xmin[i], sizeof(float), 1,
                     fp);                                                                       // xmin value
        res = fwrite(&this->ymin[i], sizeof(float), 1,
                     fp);                                                                       // ymin value
        res = fwrite(&this->zmin[i], sizeof(float), 1,
                     fp);                                                                       // zmin value
    }
...
void Compression_Valence_Component::Calculate_Edge_Color_Difference(Polyhedron &_pMesh,
    ...
    while ((hi->vertex()->Seed_Edge != 2 * _Component_ID) ||
           (hi->opposite()->vertex()->Seed_Edge != 2 * _Component_ID + 1))
        hi++;
...
void Compression_Valence_Component::Recalculate_Component_Area(Polyhedron &_pMesh, const int &_Component_ID,
                                                               int &Number_facets) {
    double Area = 0.0;
    Number_facets = 0;
    for (Facet_iterator pFacet = _pMesh.facets_begin(); pFacet != _pMesh.facets_end(); pFacet++) {
...
void
Compression_Valence_Component::Augment_Geometry_Quantization_Precision(Polyhedron &_pMesh, Arithmetic_Codec &Decoder,
                                                                       const int &Component_ID) {
    Adaptive_Data_Model Under_quantization_model(8);
...
void
Compression_Valence_Component::Diminush_Geometry_Quantization_Precision(Polyhedron &_pMesh, const int &Component_ID) {
...
QString Compression_Valence_Component::Decompress_Init(
        Polyhedron &_pMesh)//, unsigned & _Initial_file_size, const char* File_Name)
{
...
//        }
    ...
    //if (this->NumberComponents != 1)
    {
        ...
        for (int Component_number = 0; Component_number < this->NumberComponents; Component_number++) {
            ...
            while ((hi->vertex()->Seed_Edge != 2 * Component_number) ||
                   (hi->opposite()->vertex()->Seed_Edge != 2 * Component_number + 1))
                hi++;
...
//                        pHalfedge->vertex()->point() = Real_position;
                        ...
                        int Original_LAB[3];
                        Original_LAB[0] = (int) floor(
                                (Wrong_LAB[0] - this->C0_Min) / this->Color_Quantization_Step + 0.5);
                        Original_LAB[1] = (int) floor(
                                (Wrong_LAB[1] - this->C1_Min) / this->Color_Quantization_Step + 0.5);
                        Original_LAB[2] = (int) floor(
                                (Wrong_LAB[2] - this->C2_Min) / this->Color_Quantization_Step + 0.5);
                        ...
                        if (this->NumberColorQuantization[Component_number] == 0)
                            ...
                            Color_small_step = this->Color_Quantization_Step *
                                               pow(2.0, this->NumberColorQuantization[Component_number]);
...
void Compression_Valence_Component::Augment_Color_Quantization_Precision(Polyhedron &_pMesh, Arithmetic_Codec &Decoder,
                                                                         const int &_Component_ID) {
...
Point_Int Compression_Valence_Component::Change_Real_Int(const Point3d &pt, const int &Component_ID) {
    ...
    if (this->NumberChangeQuantization[Component_ID] == 0)
        ...
        Quantization_step =
                this->Quantization_Step[Component_ID] * pow(2.0, (int) this->NumberChangeQuantization[Component_ID]);
...
QString Compression_Valence_Component::Joint_Compression_Watermarking(Polyhedron &_pMesh,
    ...
    do {
        ...
        if (this->ComponentOperations[0] == this->GlobalCountOperation) {
            ...
            this->JCW_Decimation_For_Segmentation(_pMesh, _Normal_flipping, _Use_metric, _Metric_thread,
                                                  _Use_forget_metric, _Forget_value, 0);
            this->JCW_Regulation_For_Segmentation(_pMesh, _Normal_flipping, _Use_metric, _Metric_thread,
                                                  _Use_forget_metric, _Forget_value, 0);
    ...
    while (this->Decompress_count < this->ComponentOperations[0]) {
        ...
        this->JCW_Region_Mass_Center_Insert_Watermark(_pMesh, FP_Geometry, FP_Region_Number, SP_Watermarked_Position,
                                                      SP_Moved_Position, SP_Original_Position, JCW_ERROR);

        // Real refinement operations.
        this->JCW_Un_Regulation_For_Insertion(_pMesh, 0, FP_Connectivity, SP_Moved_Position, SP_Original_Position,
                                              SP_Watermarked_Position, JCW_ERROR);
        this->JCW_Un_Decimation_For_Insertion(_pMesh, 0, FP_Connectivity, SP_Moved_Position, SP_Original_Position,
                                              SP_Watermarked_Position, JCW_ERROR);
        ...
        for (Vertex_iterator pVert = _pMesh.vertices_begin(); pVert != _pMesh.vertices_end(); pVert++) {
            if (pVert->Removal_Order == this->GlobalCountOperation - this->Decompress_count) {
                Point3d WP(pVert->Watermarked_Position[0], pVert->Watermarked_Position[1],
                           pVert->Watermarked_Position[2]);
                pVert->point() = WP;
            }
        }
    ...
    this->Compression(*Base_Mesh, _Output_File_Name, _Qbit, Connectivity_size, Color_size,
                      Total_size);//, Initial_file_size);
...
// Description : Decoding of the regulation conquest
void Compression_Valence_Component::JCW_Un_Regulation(Polyhedron &_pMesh, Arithmetic_Codec &Decoder,
                                                      const int &Component_ID) {
    Init(_pMesh);
...
void
Compression_Valence_Component::JCW_Code_Difference_Histogram_Shifting(Polyhedron &_pMesh, const int &Component_ID) {
    Halfedge_iterator hi = _pMesh.halfedges_begin();
...
void
Compression_Valence_Component::JCW_Decode_Difference_Histogram_Shifting(Polyhedron &_pMesh, const int &Component_ID) {
    Halfedge_iterator hi = _pMesh.halfedges_begin();
...
int
Compression_Valence_Component::JCW_Decompress_One_Level_Without_Extraction(Polyhedron &_pMesh, const char *File_Name) {
    int res;
...
vector<double> Compression_Valence_Component::JCW_Evaluate_Robustness(void) {
    ...
    double ber =
            (double) Count_correct_watermark / ((double) Count_correct_watermark + (double) Count_incorrect_watermark);
...
void Compression_Valence_Component::JCW_Decompression_Without_Extraction_From_Sequence(Polyhedron &pMesh,
                                                                                       Polyhedron &New_mesh) {
    if (this->Process_level == 0)
        this->Write_Info(pMesh);
...
void Compression_Valence_Component::Write_Info(Polyhedron &_pMesh) {
    ...
    fprintf(this->Dec_Info, "Level %2d   #v : %8d      %6u bytes     Prog : %7.3f %%    Ratio : %9.3f\n", CLevel,
            Number_vertices, Current_file_size, prog, ratio);
